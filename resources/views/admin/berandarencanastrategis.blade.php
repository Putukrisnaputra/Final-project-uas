        ...
        <div class="shadow px-6 py-4 bg-white rounded sm:px-2 sm:py-1 sm:br-gray-100">
          <div class="grid grid-cols-12">
            <div class="col-span-6 p-4">
              <a href="{{route('rencanastrategis.create')}}"><button class="px-4 py-1 text-sm rounded text-purple-600 font-semibold border border-purple-200
            ...
            @foreach ($rencanastrategis as $item)
              ...
              <td><input type="checkbox" name="" id=""></td>
                   ...
                   <td>
                    ...
                    <td>
                        <form action="{{route('rencanastrategis.destroy',$item->id)}}" method="POST">
                        @csrf
                              @if (auth()->user()->level == 1)
                                            <form action="{{ route('rencanastrategis.destroy', $item->id) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
                                                <a href="{{ route('rencanastrategis.edit', $item->id) }}"
                                                ...
                                                <a href="{{ route('rencanastrategis.show', $item->id) }}"
